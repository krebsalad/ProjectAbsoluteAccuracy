...
void ChainManager::stateCallback(const sensor_msgs::JointStateConstPtr& msg)
{
  ...
  //!! underlying if statement not recommended! not part of official release. To revert simply remove if statement and keep error message
  sensor_msgs::JointState msg_edited;
  for (unsigned int i=0; i<msg->name.size(); i++) 
        msg_edited.name.push_back(msg->name[i]); 

  for (unsigned int i=0; i<msg->position.size(); i++) 
        msg_edited.position.push_back(msg->position[i]); 
        
  for (unsigned int i=0; i<msg->velocity.size(); i++) 
      msg_edited.velocity.push_back(msg->velocity[i]);     
  ...
  for (size_t msg_j = 0; msg_j < msg_edited.name.size(); msg_j++)
  {
    ...
    {
      if (state_.name[state_j] == msg_edited.name[msg_j])
      {
        state_.position[state_j] = msg_edited.position[msg_j];
        state_.velocity[state_j] = msg_edited.velocity[msg_j];
        break;
      }
    }
    ...
    {
      // New joint
      state_.name.push_back(msg_edited.name[msg_j]);
      state_.position.push_back(msg_edited.position[msg_j]);
      state_.velocity.push_back(msg_edited.velocity[msg_j]);
    }
  }
