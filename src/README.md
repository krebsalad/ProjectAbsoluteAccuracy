...
- The robot is calibrated using robot calibration package. Please heed the robot calibration for detailed instructions about the calibration.

- This section will cover what is to be done to calibrate your own robot using the package in THIS repo.
    1. Clone code coverage from official repo and move to your package
    ```
    git clone https://github.com/mikeferguson/code_coverage.git ~/ROS_Downloads/code_coverage
    cp -r ~/ROS_Downloads/code_coverage ~/catkin_project/src/code_coverage
    ```
    2. Copy packages abb_robot_calibration, robot_calibration and robot_calibration_msgs to your package
    ```
    cp -r ~/ROS_Downloads/abb_robot_calibration ~/catkin_project/src/abb_robot_calibration
    cp -r ~/ROS_Downloads/robot_calibration ~/catkin_project/src/robot_calibration
    cp -r ~/ROS_Downloads/robot_calibration_msgs ~/catkin_project/src/robot_calibration_msgs
    mkdir -p ~/catkin_project/src/abb_robot_calibration/include
    ``` 
    3. Build
    ```
    ...
    catkin_make
    ```
    4. Create a new calibration package for your robot
    ```
    cd ~/catkin_project/src/
    catkin_create_pkg robot_to_calibrate
    ```
    5. Create a new launch directory in the newly made package
     ```
    mkdir -p ~/catkin_project/src/robot_to_calibrate/launch
    ```
    6. Create a new launch file in the launch directory called robot_cal.launch. Heed fieldlab_cel3_calib/launch/robot_cal.launch for an example
        - This package should launch your robot
        - This package should launch your camera
        - Preferably also a robot_cal_sim.launch for fake controller launch
    7. Create a calibration directory and give it a name that is used to identify where to save your calibration
    ```
    mkdir -p ~/catkin_project/src/robot_to_calibrate/calibration_dir/abb_robot_calibration
    ```
    8. Create a config directory in the robot_to_calibrate package
    ```
    mkdir -p ~/catkin_project/src/robot_to_calibrate/config
    ```
    9. Create a capture.yaml and calibrate.yaml file based on the example fieldlab_cel3_calib and robot_calibration. (Other examples are fetch_ros and maxwell calibration)
    
    10. Edit capture.launch with your favorite editor
        - Change the arguments under capture arguments
        - Argument manual is used to choose calibration mode, if true run in manual if false a calibration bag is required
        - Argument calibration_poses_bag. Is required to run in automatic mode. can be generated using the following command. Note that you have to edit the script to      enable other feature finders than checkerboard. See line 67 of the script.
            ```
            cd ~/catkin_project/src/
            source ../devel/setup.bash
            roslaunch robot_calibration capture_poses
            ```
        - Argument output_file_ext is the name of the files that will be outputted. for example calibrated_urdf_output_file_ext.urdf ...
        - Argument calibration_output_directory is the name of directory where the calibration files will be outputted in your robot package.
        - after this you can run the calibration with
            ```
            roslaunch abb_robot_calibration capture.launch
            ```

    11. Edit calibrate.launch with your favorite editor
        - Change the arguments similar to capture.launch so that the correct directory is used.
          - after this you can run the calibration with
            ```
            roslaunch abb_robot_calibration calibrate.launch
            ```
   
    12. Use update_urdf.py to update the robot configuration with the new urdf
        - run
            ```
            cd ~/catkin_project/src/
            rosrun abb_robot_calibration update_urdf.py robot_to_calibrate/calibration_output_directory/calibrated_urdf_output_file_ext.urdf abb_irb1600_moveit_config/
            ```

    13. Enable offsets by adding a new launch to your robot launch file
