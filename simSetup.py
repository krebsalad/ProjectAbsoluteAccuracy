...
def print_log():
    print("\nSimSetup Script Log and Error list:")
    if not log:
...
pth_plannig_execution = pth_ros_downloads+"abb/abb_irb2400_moveit_config/launch/moveit_planning_execution.launch"
pth_controller_manager = pth_ros_downloads+"abb/abb_irb2400_moveit_config/launch/abb_irb2400_moveit_controller_manager.launch.xml"
pth_controllers = pth_ros_downloads+"abb/abb_irb2400_moveit_config/config/controllers.yaml"
pth_joint_names = pth_ros_downloads+abb_meta_package+"/"+abb_support_package+"/config/joint_names_"+irb_robot_name+".yaml"
pth_robot_interface_download = pth_ros_downloads+abb_meta_package+"/"+abb_support_package+"/launch/robot_interface_download_"+irb_robot_name+".launch"
...
subprocess.call(["cp", pth_plannig_execution, pth_robot_pkg+"launch/"])
subprocess.call(["cp", pth_controller_manager, pth_robot_pkg+"launch/"+irb_robot_name+"_moveit_controller_manager.launch.xml"])
subprocess.call(["cp", pth_controllers, pth_robot_pkg+"config/"])
subprocess.call(["cp", pth_joint_names, pth_robot_pkg+"config/"])
subprocess.call(["cp", pth_robot_interface_download, pth_robot_pkg+"launch/"])
...
print("\n 2. you can use moveit_setup_assistant to make changes of a existing configfile(for example :"+package_name+" package), just remember to rerun this script on that package after making changes to it with setup assistant")
exit()